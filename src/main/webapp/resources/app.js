...
function insert_car()
{
	...
	car.make=document.forms[1].make.value;
	car.model=document.forms[1].model.value;
	car.year=document.forms[1].year.value;
	...
	$.ajax({
	    ...
	    contentType : 'application/json',
	    success: function(data) {
	    	show_message("Uus auto on salvestatud");
	        console.log(JSON.stringify(data));
	    }
	  });
...
function delete_car(id)
{
	...
	$.ajax({
	    ...
	    dataType: 'json',
	    success: function(data) {
	    	show_message("Auto on kustutatud!");
	    	location.reload();
	    }
	  });
}
...
function display_cars(data)
{
	var out_data="";
	 ...
	 for(var  i in data) {
		...
		out_data = out_data + "<td><button type='button' class='btn'  onClick='javascript:delete_car(" + car.id + ")'>Kustuta</button></td>";
...
function search_car()
{
	var search_make=document.forms[2].search_make.value;
	
	$.ajaxSetup({ cache: false });
	$.ajax({

	    url: 'service/car/search/' + search_make,
	    type: "GET",
	    dataType: 'json',
	    success: function(data) {
	    	display_cars(data);
